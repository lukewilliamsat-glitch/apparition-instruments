...
export function normaliseKitDefinition(value){
 ...
 if(defaults.configurationPricing!==undefined){if(!defaults.configurationPricing||typeof defaults.configurationPricing!=='object'||Array.isArray(defaults.configurationPricing))throw new Error('Invalid configuration pricing.');for(const options of Object.values(defaults.configurationPricing)){if(!options||typeof options!=='object'||Array.isArray(options)||Object.values(options).some(price=>!Number.isSafeInteger(price)||price<0))throw new Error('Invalid configuration option price.');}}
