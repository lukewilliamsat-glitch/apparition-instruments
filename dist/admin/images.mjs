...
export function validateImage(value){if(value===null||value===undefined||value==='')return null;if(typeof value!=='string'||!/^data:image\/(png|jpeg|webp);base64,[A-Za-z0-9+/]+={0,2}$/.test(value)||value.length>Math.ceil(maximumImageBytes*4/3)+100)throw new Error('Use a PNG, JPEG or WEBP image no larger than 8 MB.');return value;}
