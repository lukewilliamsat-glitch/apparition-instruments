...
export function postalAddress(delivery={}){return [delivery.recipient,delivery.line1,delivery.line2,delivery.city,delivery.region,delivery.postcode,delivery.country==='GB'?'United Kingdom':delivery.country].filter(Boolean);}
...
export function showOrder(record,root){root.replaceChildren();const top=el('div',undefined,'order-columns'),customer=el('section'),meta=el('section');customer.append(el('h3','Customer'),definition(Object.entries(record.customer)),el('h3','Delivery'));const address=postalAddress(record.delivery);if(address.length){const block=el('address');for(const line of address)block.append(el('div',line));customer.append(block);}else customer.append(el('p','No delivery address recorded.'));if(record.paymentStatus==='paid'&&(!record.customer.name||!record.delivery.line1))customer.append(el('p','Customer or shipping details need to be retrieved from Stripe.','status-message'));meta.append(el('h3','Order details'),definition([['Created',date(record.createdAt)],['Sales channel',channels[record.channel]],['Payment',paymentLabels[record.paymentStatus]||record.paymentStatus],['Fulfilment',isPaidOrder(record)?fulfilmentLabels[record.fulfilmentStatus]||record.fulfilmentStatus:'Checkout attempt'],['External reference',record.externalReference]]));if(record.notes)meta.append(el('p',record.notes,'saved-description'));top.append(customer,meta);root.append(top,el('h3','Items'));for(const item of record.items)root.append(showItem(item));root.append(el('h3','Pricing'),definition([['Subtotal',money(record.pricing.subtotal)],['Delivery / postage',money(record.pricing.delivery)],['Order total',money(record.pricing.total)]]));const history=el('details');history.append(el('summary','Status history'));for(const event of record.statusHistory)history.append(el('p',date(event.at)+' · '+(fulfilmentLabels[event.status]||event.status)+' · '+(event.source==='admin'?'Admin':'System')));root.append(history);}
