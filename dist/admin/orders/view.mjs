...
export function showOrder(record,root){root.replaceChildren();const top=el('div',undefined,'order-columns'),customer=el('section'),meta=el('section');
 ...
 meta.append(el('h3','Order details'),definition([['Reference',record.reference],['Created',date(record.createdAt)],['Sales channel',channels[record.channel]],['Payment',paymentLabels[record.paymentStatus]||record.paymentStatus],['Fulfilment',isPaidOrder(record)?fulfilmentLabels[record.fulfilmentStatus]||record.fulfilmentStatus:'Checkout attempt'],['Confirmation email',emailState],['Total',money(record.pricing.total)],['External reference',record.externalReference]]));
