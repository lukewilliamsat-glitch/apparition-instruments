...
export function createAdminOrderRepository(transport){
 ...
 async acknowledgePartialRefund(id){const response=await transport.send('rpc/acknowledge_partial_refund',{method:'POST',body:{p_order_id:id}});if(!response.ok)throw new Error(await errorMessage(response));return response.json();},
