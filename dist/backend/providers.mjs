...
export function createAuthenticatedRepositoryTransport(auth,{config=publicBackendConfig,request=globalThis.fetch}={}){
 ...
 return Object.freeze({async send(table,{method='GET',query='',body,prefer}={}){
  if(!['components','component_internal','inventory','assemblies','assembly_bom','kit_definitions','kit_definition_internal','kit_permitted_components','orders','partial_refund_reviews','rpc/delete_unused_component','rpc/advance_order_fulfilment','rpc/acknowledge_partial_refund'].includes(table))throw new Error('Unsupported Admin repository resource.');
