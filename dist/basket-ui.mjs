...
export function totalCard(items,checkout=false){
 ...
 box.append(el('p','','Online ordering is coming soon. Your selections are saved locally for planning; no order has been placed. Kit prices and specifications are saved when added. Editing a kit applies the current options and prices.'));
 if(checkout){const b=el('button','button','Ordering coming soon');b.type='button';b.disabled=true;box.append(b,link('Return to your basket','/basket/'));}else box.append(link('Review checkout →','/checkout/','button'),link('Continue exploring','/wiring-kits/'));
 return box;
}
