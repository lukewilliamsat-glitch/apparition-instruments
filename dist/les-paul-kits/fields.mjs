...
export function renderKitFields(form){
 ...
 function group(name,title,entries,{note='',info=false}={}){
  ...
  for(const [id,option] of Object.entries(entries)){
   ...
   const capQuantity=lesPaul.requirements.filter(row=>row.source==='capacitors').reduce((sum,row)=>sum+row.quantity,0);const stock=option.component&&option.component.stock<(name==='caps'?capQuantity:option.quantity||1)?' · Out of stock':'';
 ...
 const bleed=group('bleed','Treble bleed · both volume controls',Object.fromEntries(Object.entries(lesPaul.bleed).map(([key,option])=>[key,{...option,price:option.component?option.component.kitPrice*lesPaul.requirements.find(row=>row.role==='trebleBleeds').quantity:0}])),{info:true,note:'Every upgrade price in this section covers the pair of volume controls.'});bleed.id='treble-bleed-options';bleed.setAttribute('aria-describedby','bleed-availability');
...
export function updateFieldSummaries(form,state){
 ...
 for(const group of lesPaul.builderModel.groups){for(const input of form.elements.namedItem(group.key) instanceof RadioNodeList?form.elements.namedItem(group.key):[form.elements.namedItem(group.key)]){if(!input)continue;const status=resolved.optionAvailability[group.key][input.value],disabled=status==='not-eligible',wrap=input.closest('.choice-with-info'),price=wrap.querySelector('small');input.disabled=disabled;input.setAttribute('aria-hidden',String(disabled));wrap.hidden=disabled&&!input.checked;const candidate=resolveLesPaulKit({...state,[group.key]:input.value}),potLine=candidate.pricing.lines.find(line=>line.key==='pots');const amount=group.key==='pots'?potLine.price:potLine.price-resolved.pricing.lines.find(line=>line.key==='pots').price;price.textContent=Number.isSafeInteger(amount)?upgradeLabel(amount):'Price unavailable';if(status==='out-of-stock')price.textContent+=' · Out of stock';}}
 for(const section of form.querySelectorAll('[data-option]')){
  ...
  else {const option=lesPaul[name][state[name]],input=section.querySelector(`input[value="${CSS.escape(state[name])}"]`);const line=resolved.pricing.lines.find(item=>item.key===name||name==='pots'&&item.key==='pots');const amount=line?.price??option.price;label=option.label+' · '+(option.enabled===false||input?.disabled?'Unavailable':upgradeLabel(amount));}
