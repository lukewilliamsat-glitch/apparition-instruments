...
import {defaults,normaliseKit,describeKit,specification,lesPaul,priceKit,formatKitPrice,upgradeLabel,configurationFromURL,builderURL,specificationURL} from './config.mjs';
...
let diagramView='full',diagramFocus='all',enlarged=false,editId=new URLSearchParams(location.search).get('edit'),lastComponent=null;
function readState(){return normaliseKit(Object.fromEntries(Object.keys(defaults).map(k=>[k,field(k).value])));}
function setState(value){const state=normaliseKit(value);for(const [key,val] of Object.entries(state)){field(key).value=val;if(field(key).tagName==='SELECT'&&field(key).selectedIndex<0){const option=[...field(key).options].find(o=>o.value===val);if(option)option.selected=true;}}}
...
function render(){
 const state=readState(),s=describeKit(state),mixed=state.caps==='mixed';updateFieldSummaries(form,state);$('#kit-type').textContent=lesPaul.name;
 ...
 const pricing=priceKit(state);$('#add-to-basket').disabled=!Number.isFinite(pricing.total);$('#kit-price').textContent=formatKitPrice(pricing.total);$('#builder-total').textContent=formatKitPrice(pricing.total);
 ...
 field('bleed').value=state.bleed;$('#treble-bleed-options').disabled=!modern;$('#bleed-availability').hidden=modern;
 ...
 $('#specification').value=specification(state);$('#print-kit').href=specificationURL(state);$('#view-installation').href=generatorURL(drawing,state);$('#kit-installation-guide').href=installationGuideURL(drawing,state);$('#share-fallback').hidden=true;$('#copy-status').textContent=Number.isFinite(pricing.total)?'':'This configuration contains an unavailable or unpriced option. Choose an available option before adding it to your basket.';renderDiagram(state);
}
form.addEventListener('submit',e=>e.preventDefault());form.addEventListener('change',render);field('model').addEventListener('input',()=>{const s=describeKit(readState());$('#summary-model').textContent=s.model;$('#specification').value=specification(readState());$('#print-kit').href=specificationURL(readState());$('#view-installation').href=generatorURL(drawing,readState());$('#kit-installation-guide').href=installationGuideURL(drawing,readState());$('#share-fallback').hidden=true;$('#copy-status').textContent='';});
...
$('#add-to-basket').addEventListener('click',()=>{try{addKit(readState(),editId,drawing);if(editId){location.assign('/basket/');return;}$('#copy-status').textContent='Configuration added to your basket. You can continue exploring or view your basket below.';}catch(error){$('#copy-status').textContent=error.message;}});
let initialError='';
