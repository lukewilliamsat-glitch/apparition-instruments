...
export const fields={
 ...
 bleedC:{label:'Treble bleed capacitor',unit:'nF',min:.001,max:1000},bleedR:{label:'Treble bleed resistor',unit:'kΩ',min:.001,max:10000},volume:{label:'Volume',unit:'',min:0,max:10}
};
export function validateState(state){
 const topology=topologyOf(state);if(!topology)throw new Error('Choose a supported treble bleed type and topology.');
 for(const [key,field] of Object.entries(fields)){if(key==='bleedC'&&topology==='none'||key==='bleedR'&&['none','capacitor'].includes(topology))continue;const value=state[key];if(typeof value!=='number'||!Number.isFinite(value)||value<field.min||value>field.max)throw new Error(key==='toneCap'?'Tone control capacitor: enter 0–1 µF.':`${field.label}: enter ${field.min}–${field.max} ${field.unit}.`);}
 return state;
}
