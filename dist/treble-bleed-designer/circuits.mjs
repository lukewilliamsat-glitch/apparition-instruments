...
export const defaults=Object.freeze({pickupR:8.2,pickupL:4.5,pickupC:120,volumePot:500,tonePot:500,toneCap:22,loadR:1,cableC:500,type:'none',topology:'parallel',bleedC:1,bleedR:150,volume:7,tonePosition:10});
export const fields={
 ...
 volumePot:{label:'Volume potentiometer',unit:'kΩ',min:1,max:10000},tonePot:{label:'Tone potentiometer',unit:'kΩ',min:1,max:10000},toneCap:{label:'Tone control capacitor',unit:'nF',min:0,max:1000},
 loadR:{label:'Amplifier / input load',unit:'MΩ',min:.001,max:100},cableC:{label:'Cable capacitance',unit:'pF',min:0,max:100000},
 bleedC:{label:'Treble bleed capacitor',unit:'nF',min:.001,max:1000},bleedR:{label:'Treble bleed resistor',unit:'kΩ',min:.001,max:10000},volume:{label:'Volume',unit:'',min:0,max:10},tonePosition:{label:'Tone position',unit:'',min:0,max:10}
};
export function validateState(state){
 const topology=topologyOf(state);if(!topology)throw new Error('Choose a supported treble bleed type and topology.');
 for(const [key,field] of Object.entries(fields)){if(key==='bleedC'&&topology==='none'||key==='bleedR'&&['none','capacitor'].includes(topology)||key==='tonePosition'&&state[key]===undefined)continue;const value=state[key];if(typeof value!=='number'||!Number.isFinite(value)||value<field.min||value>field.max)throw new Error(key==='toneCap'?'Tone control capacitor: enter 0–1 µF.':`${field.label}: enter ${field.min}–${field.max} ${field.unit}.`);}
 return state;
}
