...
export function configuredKitDefinitions(records,assembly=lesPaulKitAssembly){
 ...
 kit.basePrice=definition.basePrice;kit.id=assembly.id;kit.name=assembly.name;kit.family=definition.family;kit.showOnWiringKits=!!(assembly.active&&definition.showOnWiringKits);kit.builderModel=builderModel;kit.requirements=definition.defaults.requirements;kit.records=records.map(componentSnapshot);kit.defaults={...kit.defaults,...definition.defaults};kit.defaultWarnings=[];
 for(const [group,prices] of Object.entries(definition.defaults.configurationPricing||{}))for(const [key,price] of Object.entries(prices)){if(!kit[group]?.[key]||!Number.isSafeInteger(price)||price<0)throw new Error('Invalid shared Kit Definition configuration pricing.');kit[group][key].price=price;}
 if(!Number.isSafeInteger(definition.defaults.configurationPricing?.matching?.standard)||!Number.isSafeInteger(definition.defaults.configurationPricing?.matching?.precision))throw new Error('Shared Kit Definition matching prices are unavailable.');
