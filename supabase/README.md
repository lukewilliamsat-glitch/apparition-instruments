...
- The P05D-A Auth client stores standard Supabase refresh/access tokens in browser storage, refreshes expired sessions, verifies identity with `/auth/v1/user`, and signs out through `/auth/v1/logout?scope=local`. Admin repository transport obtains the user's refreshed bearer token, never a privileged key. P05B must still implement the actual Component/Inventory provider and controlled import; Admin business data remains local in P05D-A.
