...
const checkoutPage=origin+'/checkout/';
...
export async function startCheckout(req:Request,env:Environment=Deno.env,request:typeof fetch=fetch):Promise<Response>{
 ...
 if(!stripeKey||!/^sk_test_[A-Za-z0-9]+$/.test(stripeKey))return err('Stripe test checkout is not configured.',503);
 ...
 const form=new URLSearchParams({mode:'payment',client_reference_id:order.reference,success_url:checkoutPage+'?checkout=success',cancel_url:origin+'/basket/?checkout=cancel','name_collection[individual][enabled]':'true','name_collection[individual][optional]':'false','shipping_address_collection[allowed_countries][0]':'GB','metadata[order_id]':order.id,'metadata[order_reference]':order.reference,'shipping_options[0][shipping_rate_data][display_name]':'UK Standard Delivery','shipping_options[0][shipping_rate_data][type]':'fixed_amount','shipping_options[0][shipping_rate_data][fixed_amount][amount]':String(order.delivery_pence),'shipping_options[0][shipping_rate_data][fixed_amount][currency]':'gbp'});
 order.items.forEach((item:any,index:number)=>{form.set(`line_items[${index}][price_data][currency]`,'gbp');form.set(`line_items[${index}][price_data][unit_amount]`,String(item.unitPrice));form.set(`line_items[${index}][price_data][product_data][name]`,String(item.name).slice(0,150));form.set(`line_items[${index}][quantity]`,String(item.quantity));});
 ...
 const session=await stripe.json();if(!/^cs_test_/.test(session?.id||'')||!/^https:\/\/checkout\.stripe\.com\//.test(session?.url||''))return err('Stripe did not return a test checkout session.',502);
