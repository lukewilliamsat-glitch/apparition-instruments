...
assert.equal(configuredKitDefinitions(store.list())['les-paul'].bleed['bleed-duncan'].price,777);
update('bleed-duncan',{salePrice:9999});assert.equal(configuredKitDefinitions(store.list())['les-paul'].bleed['bleed-duncan'].price,777);
update('bleed-duncan',{individually:false,inKits:false});assert.equal(get('bleed-duncan').salePrice,9999);assert.equal(get('bleed-duncan').kitPrice,777);assert.equal(configuredKitDefinitions(store.list())['les-paul'].bleed['bleed-duncan'],undefined);
...
update('bleed-duncan',{kitPrice:null});assert(Number.isNaN(configuredKitDefinitions(store.list())['les-paul'].bleed['bleed-duncan'].price));
