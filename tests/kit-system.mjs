...
lesPaul.capacitors['715p-022'].price=25;assert.equal(priceKit({}).total,6049);assert.equal(priceKit({caps:'mixed',neckCap:'715p-022',bridgeCap:'225p-047'}).total,6024);lesPaul.capacitors['715p-022'].price=0;
...
const result=recommend(answers);assert(result.supported);assert.equal(result.configuration.bleed,'prs');assert(result.checks[0].includes('provisional'));assert.deepEqual(configurationFromURL(new URL(result.url,'https://example.org').search),result.configuration);
...
assert.deepEqual(kitRecord(result.configuration).configuration,result.configuration);
resetBasket();
