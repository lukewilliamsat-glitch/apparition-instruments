...
assert.equal(await repository.acknowledgePartialRefund(id,150,refundAt),true);
assert.deepEqual(calls.at(-1),{resource:'rpc/acknowledge_partial_refund',options:{method:'POST',body:{p_order_id:id,p_expected_pence:150,p_expected_refund_at:refundAt}}});
...
const guardSql=readFileSync('supabase/migrations/20260926135045_p08b2_expected_refund_review.sql','utf8');
assert.match(guardSql,/refunded_pence is distinct from p_expected_pence/);
assert.match(guardSql,/latest_refund_at is distinct from p_expected_refund_at/);
assert.match(guardSql,/drop function public\.acknowledge_partial_refund\(uuid\)/);
